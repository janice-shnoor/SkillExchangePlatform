...
function Profile() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/auth/currUser`, { credentials: 'include' })
      .then((res) => res.json())
      .then((data) => {
  ...
  if (!user) {
    return <p className="text-sm text-[var(--error)]">Unable to load profile.</p>
  }
    ...
    <div className="w-full space-y-8">
      <div className="flex items-start justify-between gap-4">
        ...
        {user.role === 'ADMIN' && (
          <Link
            to="/admin"
            className="mt-1 inline-flex shrink-0 text-sm font-medium text-[var(--primary)] hover:underline"
          >
            Admin Controls →
          </Link>
        )}
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 sm:p-8">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-[var(--primary)] hover:bg-[var(--primary-subtle)]"
          >
            <Pencil size={15} />
            Edit Profile
          </button>
        ...
        <div className="mt-8 grid gap-6 border-t border-[var(--border)] pt-8 sm:grid-cols-2">
          <div>
            <p className="text-xs uppercase tracking-wide text-[var(--text-muted)]">Full Name</p>
            <p className="mt-1.5 text-sm text-[var(--text)]">{user.name}</p>
          </div>

          <div>
            <p className="text-xs uppercase tracking-wide text-[var(--text-muted)]">Username</p>
            <p className="mt-1.5 text-sm text-[var(--text)]">@{user.username}</p>
          </div>

          <div>
            <p className="text-xs uppercase tracking-wide text-[var(--text-muted)]">Email</p>
            <p className="mt-1.5 text-sm text-[var(--text)]">{user.email}</p>
          </div>
